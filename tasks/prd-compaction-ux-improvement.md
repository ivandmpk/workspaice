...
**Root Cause Analysis:**
`compaction-detector.ts` 使用 `getModelContextWindowSync(modelId)` 从 builtin-data 获取 contextWindow，但 UI 显示的是从 provider settings 获取的 `modelInfo.contextWindow`。

例如 DeepSeek V3.2：
- builtin-data 返回 128K（通过 `deepseek-v3` 前缀匹配）
- provider settings 返回 64K（实际值）
