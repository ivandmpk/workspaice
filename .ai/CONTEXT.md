...
- Development version is now `1.0.2-beta` in root and release app manifests; keep working on `1.0.2-beta` until the user explicitly says it is released, then move development to `1.0.3-beta`. Built macOS artifacts and installed/ad-hoc-signed `/Applications/WorkspAIce.app`; verified installed `CFBundleShortVersionString=1.0.2-beta` and codesign validity.
- Renamed the session row menu label from `Move to Chat` to `Move to Workspace` in `src/renderer/components/session/SessionItem.tsx`; no behavior change.
