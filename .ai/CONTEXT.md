...
- **electron-builder + pnpm-lock.yaml coupling.** electron-builder resolves production dependencies through pnpm (because `package.json` has `packageManager: pnpm`). If `pnpm-lock.yaml` becomes stale for `release/app` (e.g. after editing `release/app/package.json` dependencies), electron-builder will package an incomplete `node_modules/` into the asar, causing runtime `Cannot find module` crashes. Always run `pnpm install` at the workspace root after changing either `package.json` or `release/app/package.json` dependencies. The `ensure-app-deps.cjs` hook removes `release/app/node_modules/` and re-installs with npm, but electron-builder's own "searching for node modules pm=pnpm" step may exclude packages that pnpm resolves differently than npm (version mismatches, peer deps). If a new direct dependency has heavyweight transitive peer deps, add those peer deps as direct dependencies too to guarantee they're packaged.
