...
export const parseLinkTool = tool({
  ...
  execute: async (input: { url: string; maxLength?: number }, { abortSignal }: { abortSignal?: AbortSignal }) => {
    ...
    // WorkspAIce AI (build-in) path: requires a license key (any tier — backend has no Pro gate).
    if (searchProvider === 'build-in') {
      const licenseKey = settingActions.getLicenseKey()
      if (!licenseKey) {
        throw WorkspAIceAIAPIError.fromCodeName(
          'parse_link via WorkspAIce AI requires a license key, but none is configured',
