...
WorkspAIce 的 AI 供应商（Provider）系统负责对接 30+ 家 AI 模型服务商，包括 OpenAI、Claude、Gemini、DeepSeek、Groq、Ollama 等。该系统采用**注册表（Registry）模式**，以 `defineProvider()` 为核心实现供应商的声明式注册，是整个 AI 调用链路的入口层。
...
```typescript
import './definitions/workspaiceai'
import './definitions/openai'
import './definitions/claude'
// ... 其余供应商
```

模块加载时 `defineProvider()` 自动执行，将供应商写入注册表。**导入顺序决定了 UI 中供应商的显示顺序**（WorkspAIceAI 始终排在首位）。
...
| provider ID mapping (`provider-mapping.ts`) | WorkspAIce ↔ models.dev ID 映射关系（单一数据源） | 不决定 provider 注册或 UI 路由 |
...
`src/shared/model-registry/provider-mapping.ts` 维护 WorkspAIce 供应商 ID 到 models.dev 供应商 ID 的映射关系：

- 支持多对一映射（如 `openai` 和 `openai-responses` 均映射到 models.dev 的 `openai`）
- 未在映射表中的供应商（Ollama、LM Studio、Azure、WorkspAIceAI 等）不参与 models.dev 富化
...
| 文件 | 用途 |
|------|------|
| `src/shared/model-registry/provider-mapping.ts` | WorkspAIce ↔ models.dev 供应商 ID 映射（单一数据源） |
...
| WorkspAIce Provider | OAuth Provider ID | Flow 类型 | 关键实现 |
|------|------|------|------|
...
`src/shared/oauth/provider-mapping.ts` 定义了 WorkspAIce provider 和 OAuth provider 的映射关系。当前只有一组共享关系：
