...
本文档描述 WorkspAIce Pro 的工具（Tool）与外部集成系统的产品设计。关于整体架构和进程模型，请参阅 [`./architecture.md`](./architecture.md)。

---

## 系统概览

WorkspAIce Pro 的工具系统为 AI 模型提供外部能力调用，使模型不仅能生成文本，还能执行搜索、读取文件、查询知识库、调用第三方服务等操作。当前工具系统由三个独立层次组成：
...
[Model Context Protocol (MCP)](https://modelcontextprotocol.io/) 是一种开放协议，允许 AI 模型通过标准化接口调用外部工具。WorkspAIce Pro 集成 MCP 的目的是让用户可以自由扩展 AI 的能力边界——无需修改应用代码即可接入新工具。
...
WorkspAIce Pro 预置了一组云端 MCP 服务器（`src/renderer/packages/mcp/builtin.ts`），通过 HTTP 传输连接到 `mcp.workspaiceai.app`：
...
内置服务器需要许可证密钥认证（通过 `x-workspaice-license` 请求头传递），用户无需额外配置即可使用。
...
|--------|------|---------|----------------------|------|
| WorkspAIce Search | `workspaice-search.ts` | ✓ | ✓ | 内置搜索，需许可证密钥（任意 tier 均可使用 parse_link） |
...
| 提供方 | 执行路径 | 失败模式 |
|--------|---------|---------|
| `build-in` (WorkspAIce AI) | 检查 `licenseKey` → 调用 `remote.parseUserLinkPro` | 缺 license 抛 `workspaice_search_license_key_required`（后端不限制 tier，任意 license 均可调用） |
...
错误抛出采用 AI/用户双层结构：`Error.message`（传给 `WorkspAIceAIAPIError` 构造器的第一参数）携带技术原因供 AI 推理（例如 "Tavily extract API returned no results for {url}"），`detail.i18nKey` 则给用户渲染本地化的友好提示。
...
工具抛出的 `WorkspAIceAIAPIError` 在 UI 上有两条独立的渲染路径：

```
tool execute throws WorkspAIceAIAPIError
        │
        ▼
...
ToolCallPartUI (ParseLinkUI / WebSearchGroupUI / GeneralToolCallUI)
  ...
  - <ToolCallErrorDetails> 读 result.errorCode
        │
        ▼
<WorkspAIceAIErrorMessage errorCode={code} />
  - WorkspAIceAIAPIError.getDetail(code) → i18nKey
  - <Trans> 渲染 OpenSettingButton / OpenExtensionSettingButton /
    OpenMorePlanButton 等可点击的跳转链接
```

`WorkspAIceAIErrorMessage`（`src/renderer/components/common/WorkspAIceAIErrorMessage.tsx`）是从 `MessageErrTips` 抽出来的可复用组件，确保同一个 WorkspAIceAI error code 在「消息级错误」和「工具级错误」两种位置展示一致。如果错误不是已知的 `WorkspAIceAIAPIError`，组件返回 `null`，`ToolCallErrorDetails` 回退显示原始错误字符串。
